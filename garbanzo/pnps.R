...
write_tsv(pnps %>% select(prot,false_beginning,`pNeg/pS-beginning`,NonSyn,Syn,pNpS) ,str_c(genome,"_pnps.tsv"))

final_tab = final_tab %>% left_join(pnps_merge,by=c("prot"="prot"))

write_tsv(final_tab %>% select(prot,start,end,frame,`#AA`,`#total_snp`,Syn,Pos,`Neg>=-2`,`Neg<-2`,AAtoSTOP, `#Syn`,`#Neg`, `#NonSyn`, `pN/pS`, `pNeg/pS` ,false_beginning,`pNeg/pS-beginning`),"summary_table.tsv")